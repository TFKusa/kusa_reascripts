...
-- InitObj_gameObjectName
-- UnRegObj_gameObjectName
-- ResetAllObj
...
local function main()
    ...
    for i = 0, num_markers - 1 do
        ...
        if not isrgn and math.abs(playPos - pos) <= marginOfError and lastMarker ~= markrgnindexnumber then
            if not markerCooldowns[markrgnindexnumber] or reaper.time_precise() - markerCooldowns[markrgnindexnumber] > cooldownPeriod then
                local namePrefix = name:sub(1, 1) == "!" and name:sub(2) or name
                ...
                if namePrefix and actionMapping[namePrefix] then
                    if processMarker(name, actionMapping[namePrefix].parts, actionMapping[namePrefix].func) then
                        ...
                        markerCooldowns[markrgnindexnumber] = reaper.time_precise()
                        break
                    end
                end
            end
