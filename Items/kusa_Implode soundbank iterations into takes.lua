...
function getUserInputs()
    local retval, userInputs = reaper.GetUserInputs("Silence Parameters", 2, "Silence Threshold,Min Silence Duration (seconds)", "0.002,0.2")
    if not retval then return end
    
    local silenceThreshold, minSilenceDuration = userInputs:match("([^,]+),([^,]+)")
    silenceThreshold = tonumber(silenceThreshold)
    minSilenceDuration = tonumber(minSilenceDuration)
    
    if not silenceThreshold or not minSilenceDuration then return end

    return silenceThreshold, minSilenceDuration
end
...
function main()
    ...
    local silenceThreshold, minSilenceDuration = getUserInputs()
